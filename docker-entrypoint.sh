...
if [ "$1" = 'mysqld' -a -z "$wantHelp" -a "$(id -u)" = '0' ]; then
	...
	if hash peer-finder 2>/dev/null; then
		peer-finder -on-start=/usr/local/bin/on-start.sh -service="${GALERA_SERVICE:-galera}"
	fi
...
if [ "$1" = 'mysqld' -a -z "$wantHelp" ]; then
	...
	if [ -f /var/lib/mysql/ibdata1 ]; then
		...
		set +e
		start_pos_opt=$(/usr/local/bin/galera-recovery.sh "${@:2}")
		set -e
		if [ $? -eq 0 ]; then
	...
	if [ ! -d "$DATADIR/mysql" ]; then
		...
		# mysql cache
		sed -i  -e "s/^\(innodb_buffer_pool_size\s*=\s*\).*$/\1${MYSQL_INNODB_BUFFER:-3072}M/"
			-e "s/^\(query_cache_size\s*=\s*\).*$/\1${MYSQL_QUERY_CACHE:-320}M/" /etc/mysql/my.cnf
